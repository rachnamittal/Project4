...
//rachna call data same as helloworld
class CallData {
public:
...
private:
  void MapProceed();
  void ReduceProceed();
//**** rachna same as hellowworld in provate section****
...
//rachna same as helloworld proceed
void CallData::Proceed() {
  if (status_ == CREATE) {
    ...
    //rachna here splitting for map and reduce calls
    if (request_.is_map()) {
      // map function
      // init output_num for hash the key into R regions
      std::cout << "Receive map task from master ..." << std::endl;
      MapProceed(); //reply_.set_message(prefix + request_.name()); for actual processing
...
/* CS6210_TASK: ip_addr_port is the only information you get when started.
  ...
  //rachna this is in void Run(). HandleRpcs(); is missing in this block
